...
class MyTrips extends Component {
    ...
    componentWillMount() {
        ...
        axios.post(rootURL + '/getCurrentUserBookingDetails', data)
            .then(response => {
    ...
    checkIn = (bookingId, roomId) => {
        ...
        axios.post(rootURL + '/guestCheckIn', data)
            .then(response => {
                ...
                if (response.status === 200) {
                    console.log(response.data);
                    swal("Success", "SUccessfully Checked In, amount will be debited", "success")
                    const data1 = {
                        customerId: localStorage.getItem("userId")
                    }
                    //404 no bookings for past/future/current
    ...
    // Need to integrate CHeckout API here
    checkOut = (bookingId, roomId) => {
        ...
        axios.post(rootURL + '/guestCheckout', data)
            .then(response => {
                let data = ""
                if (response.status === 200) {
                    console.log(response.data);
                    swal("Success", "Successfully, Checked out", "success")
                    const data1 = {
                        customerId: localStorage.getItem("userId")
                    }
                    this.setState({
    ...
    CancelBooking = (bookingId) => {
        ...
        axios.post(rootURL + '/cancelBookingGuest', data)
            .then(response => {
                ...
                if (response.status === 200) {
                    console.log(response.data);
                    swal("Success", "Successfully cancelled booking", "success")
                }
                else if (response.status === 201) {
                    console.log(response.data);
                    penalty = "" + response.data
                    swal("Success", penalty, "warning")
                }
                const data1 = {
                    customerId: localStorage.getItem("userId")
                }
    ...
    render() {
        ...
        if (this.state.redirect === true) {
            redrirectVar = <Redirect to="/my-trips/previous" />
        }
        ...
        let tripDetails = "No Current Bookings For You"
        ...
        if (this.state.myTrips.length > 0) {
            tripDetails = this.state.myTrips.map((trip, index) => {
                ...
                if (trip.bookingStatus === "new") {
                    checkStatus = <button onClick={() => this.checkIn(trip.bookingId, trip.roomId)} className="btn btn-warning" id="button">Check In</button>
                }
                else if (trip.bookingStatus === "checkin") {
                    checkStatus = <button onClick={() => this.checkOut(trip.bookingId, trip.roomId)} className="btn btn-warning" id="button">Check out</button>
                }
                return (
